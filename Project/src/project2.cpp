  ...
  for (std::vector<hot_loop_data_t*>::iterator it = hot_loops.begin() ; it != hot_loops.end(); ++it){
    ...
    for (INS ins = RTN_InsHead(rtn); INS_Valid(ins); ins = INS_Next(ins)){
      if(INS_IsDirectBranch(ins)){
        if (INS_DirectBranchOrCallTargetAddress(ins) == (*it)->target_addr && INS_HasFallThrough(ins) == true){
...
/*****************************************/
/* find_iter_update_ins(start_addr, end_addr) */
/*****************************************/
INS find_iter_update_ins(RTN rtn, ADDRINT start_addr, ADDRINT end_addr){
  INS iter_update_ins = RTN_InsHead(rtn);
  for (INS ins = RTN_InsHead(rtn); INS_Valid(ins); ins = INS_Next(ins)) {
      ...
      if(INS_Address(ins) < end_addr){
          ...
          if ((inst_iclass == XED_ICLASS_ADD) || (inst_iclass == XED_ICLASS_SUB ))
          {
              iter_update_ins = ins;
          }
      }
  ...
  return iter_update_ins;
}

/******************************************************************************/
/* std::vector <xed_reg_enum_t>  get_ins_registers(xed_decoded_inst_t* xedd)  */
/******************************************************************************/
std::vector <xed_reg_enum_t>  get_ins_registers(xed_decoded_inst_t* xedd) 
{
    ...
    for( i=0; i < noperands ; i++) { 
      ...
      switch(op_name) {
        ...
        {
          if (xed_decoded_inst_get_reg(xedd, op_name) != XED_REG_INVALID &&
          xed_decoded_inst_get_reg(xedd, op_name) != XED_REG_RFLAGS){
            r.push_back(xed_decoded_inst_get_reg(xedd, op_name));
            break;
          }
        }
        default:
...
/**************************************************/
/* int encode_cmp(INS ins) */
/**************************************************/
int encode_cmp(INS ins, UINT64 rem, UINT64 cnt_seen){
  int rc;
  ...
  if (inst_iclass == XED_ICLASS_CMP){
    ...
    if (get_immediate(xedd2, &x)){  
      ...
      xed_inst2(&enc_instr, dstate, 
                          XED_ICLASS_CMP, 32,
                          xed_mem_bd(base_reg, xed_disp(mem_disp, 32), 32),
                          xed_imm0(rem, 32)
                          );
...
/*****************************************/
/* void get_loop_inst(RTN rtn, ADDRINT start_addr, ADDRINT end_addr, vector<INS>& o_loop_ins) */
/*****************************************/
void get_loop_instructions(RTN rtn, ADDRINT start_addr, ADDRINT end_addr, vector<INS>& o_loop_ins, INS& last_ins_in_loop)
{
  ...
  for (INS ins = RTN_InsHead(rtn); INS_Valid(ins); ins = INS_Next(ins)) {
    ...
    else if (INS_Address(ins) > end_addr){
      break;
    }

    // Get the last instruction in loop
    else if (INS_Address(ins) == end_addr){
      last_ins_in_loop = ins;
    }

    // if we are currently in the loop
    else
    {
      o_loop_ins.push_back(ins);
    }
  }
...
int encode_loop(vector<INS>& loop_ins){

  int rc = 0;

  for (std::vector<INS>::iterator it = loop_ins.begin() ; it != loop_ins.end(); ++it){
    if( (rc = encode_ins(*it)) != 0){
      cerr << "Error while encoding instruction" <<endl;
      return rc;
    }
  }
...
/*****************************************/
/* void unroll_single_loop(RTN rtn) */
/*****************************************/

int unroll_single_loop (vector<INS>& loop_instructions, INS last_ins_in_loop, UINT32 reminder){
  ...
  // First extract the reminder
  for (UINT32 i = 0; i < reminder; i++){
    if ((rc = encode_loop(loop_instructions)) != 0){
      cerr<<"Error while encoding instruction";
      return rc;
    }
  }

  cout<<"UNROLL_LVL is:"<<UNROLL_LVL<<endl;

  // Then extract the rolling
  for (UINT32 i = 0; i < UNROLL_LVL; i++){
    if ((rc = encode_loop(loop_instructions)) != 0){
      cerr<<"Error while encoding instruction";
      return rc;
    }
  ...
  if ((rc = encode_ins(last_ins_in_loop)) != 0){
    return rc;
  }
...
/* void unroll(RTN rtn) */
/*****************************************/
void unroll_routine(RTN rtn){

  cout << RTN_Name(rtn) << endl << endl;
 

  ADDRINT start_addr = 0;
  ADDRINT end_addr = 0;
  UINT64 cnt_seen = 0; // TODO: get the number of times loop invoked
  

  for (std::vector<hot_loop_data_t*>::iterator it = hot_loops.begin() ; it != hot_loops.end(); ++it){
    if ((*it)->rtn_addr == RTN_Address(rtn)){
      start_addr = (*it)->target_addr;
      end_addr = (*it)->end_addr;
      cnt_seen = (*it)->cnt;
      cout << "Found hot lpop for: " << RTN_Name(rtn)<<endl;
      cout<<"start address: "<< std::hex<< start_addr<<endl;
      cout<<"end address: "<< std::hex<< end_addr<<endl;
      cout<<"count: "<< cnt_seen<<endl;
      break;
    }
  }

 UINT32 reminder = cnt_seen % UNROLL_LVL;

  int rc = 0;

  // Vector that contains the loop instructions (without end_addr!)
  vector<INS> loop_ins;
  INS last_ins_in_loop;
  get_loop_instructions(rtn, start_addr, end_addr, loop_ins, last_ins_in_loop);

  bool flag = false;

  cout<<"loop size is: "<<std::dec<<loop_ins.size()<<endl;
  cout<<"last instruction address is: "<<std::hex<<INS_Address(last_ins_in_loop)<<endl;

  for (INS ins = RTN_InsHead(rtn); INS_Valid(ins); ins = INS_Next(ins)) {
    if(INS_Address(ins) < start_addr || INS_Address(ins) > end_addr){
      if( (rc = encode_ins(ins)) != 0){
        cerr << "Error while encoding instruction" <<endl;
        break;
      }
    }
    ...
    {
      if(flag == false){
        unroll_single_loop(loop_ins, last_ins_in_loop, reminder);
        flag = true;
      }
...
void unroll(RTN rtn){
  int rc;
  vector<INS> unroll_ins;
  cout << RTN_Name(rtn) << endl << endl;
  hot_loop_data_t* curr_loop = hot_loops.front();
  ...
  for (INS ins = RTN_InsHead(rtn); INS_Valid(ins); ins = INS_Next(ins)) {
    if(INS_Address(ins) < start_addr){
      ...
      //unroll 
      for(UINT32 i=0; i < UNROLL_LVL - 1; i++){
        for (std::vector<INS>::iterator it = unroll_ins.begin() ; it != unroll_ins.end(); ++it){
          INS last_ins = unroll_ins.back();
          if(INS_Address(*it) == INS_Address(last_ins)){
            rc = encode_cmp(*it, rem, cnt_seen);
            if(rc != 0){
              break;
            }
          }
      ...
      // encode conditional branch at the end of the loop
      rc = encode_ins(ins);
      if(rc != 0){
        break;
      }
      ...
      hot_loops.erase(hot_loops.begin());
      curr_loop = hot_loops.front();
      start_addr = curr_loop->target_addr;
      end_addr = curr_loop->end_addr;
      cnt_seen = curr_loop->cnt;
      rem = cnt_seen % UNROLL_LVL;
    }

    else{
      if (INS_Address(INS_Next(ins)) == end_addr){
        if(rem > 0){
          rc = encode_cmp(ins, rem, cnt_seen);
          if(rc != 0){
            break;
          }
        }
      ...
      else if(INS_Address(INS_Next(ins)) != end_addr){
        rc = encode_ins(ins);
        if(rc != 0){
          break;
        } 
      }
      unroll_ins.push_back(ins);
    }
  }
}
...
/*****************************************/
bool loop_cmp(hot_loop_data_t* lhs, hot_loop_data_t* rhs)
{
    return lhs->target_addr < rhs->target_addr; // here go your sort conditions
}
...
{
  std::sort(hot_loops.begin(), hot_loops.end(), loop_cmp);
    ...
    if (!SEC_IsExecutable(sec) || SEC_IsWriteable(sec) || !SEC_Address(sec)) continue;
      for (RTN rtn = SEC_RtnHead(sec); RTN_Valid(rtn); rtn = RTN_Next(rtn)){  
        ...
        if(rtn_is_hot(rtn)){
          ...
          //if(!rtn_name.compare("mainGtU")) continue;
          if(rtn_name == "mainGtU" || rtn_name == "copy_input_until_stop"
              || rtn_name == "BZ2_blockSort")
          //if(rtn_name == "mainGtU" )
          {
